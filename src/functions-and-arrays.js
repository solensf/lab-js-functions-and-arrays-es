...
const meLoInvento = ['uno', 'dos', 'patata', 'astrolopitecus', 'hola', 'pan'];
...
function averageNumbers(numberArray) {
  ...
  for (let i = 0; i < numberArray.length; i++) {
    suma = suma + numberArray[i];
    average = suma / numberArray.length;
  }
  return average;
}
...
function averageWordLength(words) {
  ...
  let sumaWordLengt = 0;
  let wordsLengthAverage = 0;
  for (let i = 0; i < words.length; i++) {
    sumaWordLengt = sumaWordLengt + words[i].length;
    wordsLengthAverage = sumaWordLengt / words.length;
  }
  return wordsLengthAverage;
}

// Bonus - Iteration #4.1
function avg(wordMixed) {
  if (!wordMixed.length) return null;
}
